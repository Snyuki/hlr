...
static
void
calculateGS (struct calculation_arguments const* arguments, struct calculation_results* results, struct options const* options, int start, int end, int rank, int size)
{
	int i, j;           /* local variables for loops */
	int m1, m2;         /* used as indices for old and new matrices */
	double star;        /* four times center value minus 4 neigh.b values */
	...
	int stop = 0; // Cascading Variable for communicating the stop iteration when in precision mode
	int reminder = 0; // Reminder Variable for the last process to not send stop again
	...
	while (term_iteration > 0)
	{
		...
		if(rank != 0)
		{
			MPI_Recv(Matrix_In[0], N+1, MPI_DOUBLE, rank-1, term_iteration, MPI_COMM_WORLD, MPI_STATUS_IGNORE); // Waits to recieve the last line from the previous process in every iteration
			MPI_Recv(&maxResiduum, 1, MPI_DOUBLE, rank-1, term_iteration, MPI_COMM_WORLD, MPI_STATUS_IGNORE); // Waits to recieve the maxResiduum from the previous process in every iteration
			if(options->termination == TERM_PREC)
			{
				MPI_Recv(&stop, 1, MPI_INT, rank-1, term_iteration, MPI_COMM_WORLD, MPI_STATUS_IGNORE); // If in precision mode, waits to recieve the stop variable from the previous process in every iteration
			}
		}
		...
		for (i = 1; i < range + bufferlines; i++)
		{
			// Checks if its not the last process, the line iteration is 2, and the process is already running for at least one iteration
			if((rank != size - 1) && (i == 2) && (term_iteration != (int)options->term_iteration))
			{
				// Before beginning with the calculation of the second line, waits to recieve the first line from the next process in every iteration
				// This is done in the second line, because it temporally corresponds with the moment in which the next process finishes its first line
				MPI_Recv(Matrix_In[range + bufferlines], N+1, MPI_DOUBLE, rank+1, term_iteration, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			}
			...
			// Checks if its not the first process, the line iteration is 1, and the process has not already gotten the order to stop (because otherwise there would be no previous process anymore)
			if((rank != 0) && (i == 1) && (term_iteration != 1) && (stop != 1))
			{
				// Immediately after calculating the first line, sends the first line to the previous process in each iteration
				// This is done right away, because the previous process can then continue with the next iteration
				MPI_Ssend(Matrix_Out[1], N+1, MPI_DOUBLE, rank-1, term_iteration - 1, MPI_COMM_WORLD);
			}
		}
		...
		if(rank == size-1)
		{
			// Only the last rank sets the stat precision in the results, because only it has the maxResiduum of the entire matrix
		...
		// Executed only if in precision mode and the stop signal is not 1 (Because otherwise, the first process would wait again for a signal that only gets sent once)
		if(options->termination == TERM_PREC && stop == 0)
		{
			// Checks if its the first process, and the program is already running for at least (term_iteration - nprocs) iterations (because otherwise the last process hasnt even started its calculation)
			if(rank == 0 && term_iteration <= (int)options->term_iteration - (size-1))
			{
				MPI_Recv(&stop, 1, MPI_INT, size-1, term_iteration, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			}

			// Checks if its the last process, and no previous stop signal has been sent (because otherwise, the first process would already have stopped its calculation)
			if(rank == size-1 && reminder == 0)
			{
				if(maxResiduum < options->term_precision)
				{
					// If the precision has been reached, the last process sends the stop signal 1 to the first one, resulting in a cascading calculation end
					// The last process then sets its reminder and continues calculating, until it recieves the real stop signal through the cascade again
					stop = 1;
					MPI_Ssend(&stop, 1, MPI_INT, 0, term_iteration - (size-1), MPI_COMM_WORLD);
					stop = 0;
					reminder = 1;
				}
				else
				{
					// If the precision has not been reached, the last process sends the stop signal 0 to the first one, and everything continues as normal
					MPI_Ssend(&stop, 1, MPI_INT, 0, term_iteration - (size-1), MPI_COMM_WORLD);
				}
			}
		}
		...
		if(rank != size-1)
		{
			MPI_Ssend(Matrix_Out[range + bufferlines - 1], N+1, MPI_DOUBLE, rank+1, term_iteration, MPI_COMM_WORLD); // Sends the last line to the next process in every iteration
			MPI_Ssend(&maxResiduum, 1, MPI_DOUBLE, rank+1, term_iteration, MPI_COMM_WORLD); // Sends the maxResiduum to the next process in every iteration
			if(options->termination == TERM_PREC)
			{
				MPI_Ssend(&stop, 1, MPI_INT, rank+1, term_iteration, MPI_COMM_WORLD); // If in precision mode, sends the stop variable to the next process in every iteration
			}
		}
		...
		if(stop == 1)
		{
			term_iteration = 0; // If the stop variable is 1, ends the calculation after this iteration
		}
	}
...
displayMatrix (struct calculation_arguments* arguments, struct calculation_results* results, struct options* options, int start, int end, int rank, int size)
{
    ...
    for (y = 0; y < 9; y++)
	{
		for (x = 0; x < 9; x++)
		{
			...
			{
				printf("%7.12f ", Matrix[global_index - start + add][x * (interlines + 1)]);
			}
		}
...
displayMatrixSequential (struct calculation_arguments* arguments, struct calculation_results* results, struct options* options)
{
	...
	for (y = 0; y < 9; y++)
	{
		for (x = 0; x < 9; x++)
		{
			printf ("%7.12f ", Matrix[y * (interlines + 1)][x * (interlines + 1)]);
		}
...
mainGSParallel(struct calculation_arguments* arguments, struct calculation_results* results, struct options* options, int rank, int size)
{
	...
	gettimeofday(&start_time, NULL);
	calculateGS(arguments, results, options, start, end, rank, size);
	gettimeofday(&comp_time, NULL);
