...
initMatrices (struct calculation_arguments* arguments, struct options const* options, int start, int end, int rank, int size)
{
	...
	if (options->inf_func == FUNC_F0)
	{
		for(i = 0; i < range + bufferlines; i++)
		{
			for (j = 0; j < N; j++)
			{
				for (uint64_t m = 0; m < arguments->num_matrices; m++)
				{
					Matrix[m][i][0] = 1 + (1 - (h * (start+i))); // Linke Kante
					Matrix[m][range - i][N] = h * (start+i); // Rechte Kante

					if(rank == 0) {
						Matrix[m][0][N - j] = 1 + h * j; // Obere Kante
					}
					if(rank == size-1) {
						Matrix[m][range][j] = 1 - (h * j); // Untere Kante
					}
				}
			}
		}
...
int
calcStart (int N, int rank, int size)
{
	int lines = (N+1)/size;
	int extra = (N+1)%size;
...
int
calcEnd (int N, int rank, int size)
{
	int lines = (N+1)/size;
	int extra = (N+1)%size;
	...
	printf("Rank %d contains %d valid lines, from index %d to index %d\n", rank, endline-startline+1, startline, endline);
...
calculate (struct calculation_arguments const* arguments, struct calculation_results* results, struct options const* options, int start, int end, int rank, int size)
{
	...
	while (term_iteration > 0)
	{
		...
		for (i = 1; i < range + bufferlines; i++)
		{
			...
			int realline = start + i + subtract;

			if(term_iteration == 1) {
				//printf("Rank %d calculates for realline %d\n", rank, realline);
			}
			if (options->inf_func == FUNC_FPISIN)
			{
				fpisin_i = fpisin * sin(pih * (double)realline);
			}
...
main (int argc, char** argv)
{
	...
	int start = calcStart(N, rank, size);
	int end = calcEnd(N, rank, size);
