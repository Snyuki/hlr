...
calculate (struct calculation_arguments const* arguments, struct calculation_results* results, struct options const* options, int start, int end, int rank, int size)
{
	...
	double maxResiduum[options->number];	 	/* maximum residuum values of a slave in iteration. Each thread should put its maxResiduum in maxResiduum[thread_id] */
	
	double global_maxResiduum;  /* Max residuum over all processes */
	...
	// Init the maxResiduum Array with zeros
	for (uint64_t l = 0; l < options->number; l++)
	{
		maxResiduum[l] = 0.0;
	}
	...
	#pragma omp parallel \
			...
			shared(arguments, results, options, N, h, pih, fpisin, m1, m2, term_iteration, maxResiduum, rank, size, bufferlines, range, global_maxResiduum) \
			default(none) \
			num_threads(options->number)
	#endif
	{
		int thread_id = 0;										// If OpenMP is not used, set thread id to 0 because it sequentially runs on one thread
		#ifdef _OPENMP
		thread_id = omp_get_thread_num();						// Get thread id from OpenMP
		#endif
		while (term_iteration > 0)
		{
			...
			// Darf nur ein OPM-Thread machen!!
			if(rank != 0)
			{
			...
			for (i = 1; i < range + bufferlines; i++)
			{
				...
				for (j = 1; j < N; j++)
				{
					...
					if (options->termination == TERM_PREC || term_iteration == 1)
					{
						...
						maxResiduum[thread_id] = (residuum < maxResiduum[thread_id]) ? maxResiduum[thread_id] : residuum;
					}
			...
			/* sync threads */
			// printf("Thread %d: Waiting for other threads to finish iteration.\n", thread_id);
			#ifdef _OPENMP
			#pragma omp barrier
			#endif

			// Calculation of one iteration is done

			#ifdef _OPENMP
			#pragma omp single
			#endif
			{
				// Do only if precision termination or last iteration: Reduce max residuum of all threads to on value
				if (options->termination == TERM_PREC || term_iteration == 1)
				{
					double maxResiduum_of_all_threads = 0;

					// Sync residuum with threads
					for(uint64_t l = 0; l < options->number; l++)
					{
						maxResiduum_of_all_threads = (maxResiduum[l] < maxResiduum_of_all_threads) ? maxResiduum_of_all_threads : maxResiduum[l];
					}

					// Sync residuum with processes
					MPI_Reduce(&maxResiduum_of_all_threads, &global_maxResiduum, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
...
main (int argc, char** argv)
{
	...
	int use_sequential = (size == 1 || (options.method == METH_GAUSS_SEIDEL)) ? 1 : 0;
